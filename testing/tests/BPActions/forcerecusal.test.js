...
describe("Force recusal case Telos Arbitration Smart Contract Tests", () => {
    ...
    it("force recusal", async () => {
        expect.assertions(5);
        
        await arbitration.contract.forcerecusal({ case_id: "0", rationale: "Recusal", arbitrator: "user3" },
            [{
    ...
    it("fails if case not found", async () => {
        await expect(arbitration.contract.forcerecusal({ case_id: "5", rationale: "Recusal", arbitrator: "user3" },
            [{
              actor: admin.accountName,
              permission: "active"
    ...
    it("fails if trying to recuse a finished case", async () => {
        await arbitration.loadFixtures("casefiles", {
         ...
         await expect(arbitration.contract.forcerecusal({ case_id: "1", rationale: "Recusal", arbitrator: "user3" },
            [{
              actor: admin.accountName,
              permission: "active"
            }])).rejects.toThrow("Unable to force recusal if case has not started or if is resolved");
    });
    ...
    it("fails if arbitrator is not selected for the case", async () => {
        await expect(arbitration.contract.forcerecusal({ case_id: "0", rationale: "Recusal", arbitrator: "user2" },
            [{
              actor: admin.accountName,
              permission: "active"
    ...
    it("fails if user other than admin tries", async () => {
        await expect(arbitration.contract.forcerecusal({ case_id: "0", rationale: "Recusal", arbitrator: "user3" },
            [{
