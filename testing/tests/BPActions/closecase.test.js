...
describe("Close case Telos Arbitration Smart Contract Tests", () => {
    ...
    beforeEach(async () => {
        ...
        await arbitration.loadFixtures("casefiles", {
            arbitration: [{
                ...
                case_ruling: 'QmTtDqWzo1TXU7pf2PodLNjpcpQQCXhLiQXi6wZvKdFe5a',
                recusal: "",
    ...
    it("closes a case", async () => {
        await arbitration.contract.closecase({
            case_id: "0",
        }, [{
        ...
        expect(config.reserved_funds).toEqual("100.0000 TLOS");
        expect(config.available_funds).toEqual("15.0000 TLOS");
    });

    it("fails if case not found", async () => {
         await expect(arbitration.contract.closecase({
            case_id: "2",
         },
            [{
              actor: admin.accountName,
    ...
    it("fails if case is not in decision status", async () => {
        ...
        await expect(arbitration.contract.closecase({
            ...
            [{
              actor: admin.accountName,
              permission: "active"
            }])).rejects.toThrow("Case status must be ENFORCEMENT");
    });
    
    it("fails if user other than admin tries to close the case", async () => {
        await expect(arbitration.contract.closecase({
            case_id: "0",
        },
            [{
